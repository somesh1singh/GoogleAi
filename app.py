...
# 1. HARDCODED BASELINE DATA INGESTION ENGINE
# If no custom file is attached, the app automatically falls back to your synchronized master ledger
@st.cache_data
...
# Dynamic computation for cash release strategy
cash_unlocked = df[df["Command"].str.contains("EXIT|TRIM|LOSS", na=False)]["Current_Value"].sum()

m1, m2, m3, m4 = st.columns(4)
m1.metric("Deployed Capital Base", f"₹{total_deployed:,.2f}")
m2.metric("Active Portfolio Value", f"₹{total_value:,.2f}")
m3.metric("Net Unrealized Delta P&L", f"₹{total_pnl:,.2f}", delta=f"{(total_pnl/total_deployed)*100:.2f}%", delta_color="inverse")
