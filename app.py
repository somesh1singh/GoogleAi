...
# =========================================================================================
# EXPERT BINARY EXPORT BUTTON PIPELINES (THE STRATEGIC ENHANCEMENT)
# =========================================================================================
@st.cache_data
def convert_to_excel_stream(data_df, sc_df, deployed, value, pnl, unlocked):
    ...
    ws1["A3"] = "Total Deployed Capital Base:"; ws1["D3"] = deployed; ws1["D3"].number_format = "₹#,##,##0.00"; ws1["D3"].font = b_font
    ws1["A4"] = "Active Current Portfolio Value:"; ws1["D4"] = value; ws1["D4"].number_format = "₹#,##,##0.00"; ws1["D4"].font = b_font
    ws1["A5"] = "Net Unrealized Capital Drag:"; ws1["D5"] = pnl; ws1["D5"].number_format = "₹#,##,##0.00"; ws1["D5"].font = b_font
    ws1["A6"] = "Net Unlocked Reinvestment Liquidity:"; ws1["D6"] = unlocked; ws1["D6"].number_format = "₹#,##,##0.00"; ws1["D6"].font = b_font
    ...
    for idx, r in data_df.iterrows():
        row_num = idx + 3
        ws2.cell(row=row_num, column=1, value=r["Ticker"]).font = b_font
        ws2.cell(row=row_num, column=2, value=r["Units"])
ws2.cell(row=row_num, column=3, value=r["Avg_Cost"])
ws2.cell(row=row_num, column=4, value=r["LTP"])
ws2.cell(row=row_num, column=5, value=f"=B{row_num}*C{row_num}")
ws2.cell(row=row_num, column=6, value=f"=B{row_num}*D{row_num}")
ws2.cell(row=row_num, column=7, value=f"=F{row_num}-E{row_num}")
ws2.cell(row=row_num, column=8, value=r["Delta_ROCE_Spread"])
ws2.cell(row=row_num, column=9, value=r["CCC"])
ws2.cell(row=row_num, column=10, value=r["Command"]).font = b_font
for col_f in: ws2.cell(row=row_num, column=col_f).number_format = "₹#,##,##0.00"
ws2.cell(row=row_num, column=8).number_format = "+0.00%;-0.00%"
for c_j in range(1, 11): ws2.cell(row=row_num, column=c_j).border = border
for ws in [ws1, ws2]:
