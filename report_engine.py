...
from openpyxl.utils import get_column_letter  # FIXED: Explicitly imported utility to prevent the line 105 crash
import io
...
def convert_to_excel_stream(data_df, sc_df, deployed, value, pnl, unlocked):
    ...
    # Native Excel grid lines explicitly forced ON
    ...
    # Total Summary Rollup Row inside Excel Sheet 2
    ...
    # Auto-adjust column tracking width natively using the imported get_column_letter API
    for ws in [wb["1. Lead Risk Dashboard"], wb["2. 51-Stock Forensic Ledger"]]:
        for col in ws.columns:
            max_len = max(len(str(cell.value or '')) for cell in col)
            ws.column_dimensions[get_column_letter(col.column)].width = max(max_len + 4, 13)
