...
def convert_to_excel_stream(data_df, sc_df, deployed, value, pnl, unlocked):
    ...
    # FIXED: Replaced the tuple 'for col in ws.columns' loop with an explicit, robust integer index loop
    # This prevents the openpyxl 'tuple object has no attribute width' crash completely
    for ws in [ws1, ws2]:
